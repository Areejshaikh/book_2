...
def main():
    ...
    5. Store in Qdrant (if available) or save locally as fallback
    """
    ...
    try:
        ...
        # 7. Try to save all chunks to Qdrant
        if all_chunks_with_embeddings:
            logger.info(f"Attempting to save {len(all_chunks_with_embeddings)} chunks to Qdrant")
            
            try:
                # Try to initialize Qdrant store and save
                ...
                if not qdrant_store.create_collection():
                    ...
                    # Save embeddings locally as fallback
                    save_embeddings_locally(all_chunks_with_embeddings)
                else:
                ...
                logger.error(f"Error with Qdrant connection: {str(e)}")
                # Save embeddings locally as fallback
                save_embeddings_locally(all_chunks_with_embeddings)
        else:
...
def save_embeddings_locally(chunks_with_embeddings: List[Dict]):
    """
    Save embeddings locally as a fallback when Qdrant is not available
    """
    import json
    ...
    filename = f"embeddings/embeddings_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    
    with open(filename, 'w', encoding='utf-8') as f:
        json.dump(chunks_with_embeddings, f, indent=2, ensure_ascii=False)
    
    logger.info(f"Saved {len(chunks_with_embeddings)} embeddings locally to {filename}")
